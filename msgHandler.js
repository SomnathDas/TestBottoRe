...
const yt = require('./yt') //yt
const msgFilter = require('./lib/msgFilter')
const akaneko = require('akaneko')
const fetch = require('node-fetch')
const bent = require('bent')
const source = require('./lib/sauce.js')
...
✨️ *Japanese Title:* ${data.japaneseTitle}

🎆️ *Episode:* ${data.episodes}

💌️ *Rating:* ${data.rating}

❤️ *Score:* ${data.score}

💚️ *Synopsis:* ${data.synopsis}
	...
	if (!isCmd && isGroupMsg) return console.log('[RECV]', color(time, 'yellow'), 'Message from', color(pushname), 'in', color(name))
        ...
        const isUrl = new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/gi)
          ...
          switch (command) {
            ...
            case 'stiker':
                if (isMedia) {
                    ...
                    await client.sendImageAsSticker(from, imageBase64)
                          ...
                          sendSticker.sendAnimatedSticker(quotedMsg)
                          } else {
                          await client.reply(from, 'The given file is too large for converting', id)
                          }
                } else {
            ...
            break
        /*case 'test':
            await client.reply(from, JSON.stringify(cantdelete), id)*/
        /*case 'donate':
            client.reply(from, '...', '...')
            break*/
        case '!stickergif':
      ...
      case 'aiquote' :
       ...
       case 'translate':
	    ...
	    if (!msgFilter.isHai) {
		client.reply(from, 'Take a break baka, You can only summon one haigusha in the span of 45 Seconds')
	    } else {
            const groupthing = message.chat.id
	    msgFilter.addHai(from)
            const waifu = await waifuclient.getRandom()
            const wname = `${waifu.data.name}`
            var wan = wname.replace(' (husbu)', '')
            await client.sendFileFromUrl(from, waifu.data.display_picture, 'haugusha.jpg', `❤️ *Name : ${wan}*\n\n💎️ Description : ${waifu.data.description}\n\n💚️ Source : ${waifu.data.series.name}\n\n✨️ URL: ${waifu.data.url}`, id)
            const wanarr = [wan]
            await fs.writeFile(`./lib/ssdata/groups/${groupfile}`, JSON.stringify(wanarr))
	    } */
	    client.reply(from, `[Disabled] ❌️`, id)
            break 
        case 'claim' :
            if (!isGroupMsg) return client.reply(from, 'Baka!, This command can only be used in groups', message.id)
...
Plan to Read: ${jikan.manga_stats.plan_to_read}`
	...
	case 'delete':
        ...
        case 'demote':
            ...
            break
        /*case 'join': DISABLED AS REQUESTED
